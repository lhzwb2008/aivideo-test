...
def _proxy_opener() -> urllib.request.OpenerDirector | None:
    proxy = _env("HTTPS_PROXY") or _env("https_proxy") or _env("YOUTUBE_HTTP_PROXY")
    if not proxy:
        return None
...
def _get(url: str, *, headers: dict[str, str] | None = None, timeout: float = 120) -> bytes:
    hdrs = {"User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7) aivideo-test/1.0"}
    if headers:
        hdrs.update(headers)
...
def fetch_clip_for_query(query: str, *, stem: str, index: int) -> Path | None:
    ...
    videos = search_videos(query, per_page=8)
    for v in videos:
